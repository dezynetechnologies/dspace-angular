...
export class SamvadAIComponent{
  ...
  getSelectedNode(node: FlatNode) {
    if(this.selectedNode && this.selectedNode.indexOf(node) > -1){
      const index = this.selectedNode.indexOf(node, 0);
      ...
      // botMessage.textContent = "The documents will be fetched from " + node.name;
