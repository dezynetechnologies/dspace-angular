...
import { SearchOptions } from '../../../shared/samvad-ai/models/search-options.model';
import { PaginatedSearchOptions } from '../../../shared/samvad-ai/models/paginated-search-options.model';
import { SearchFilter } from '../../../shared/samvad-ai/models/search-filter.model';
...
import { SearchConfig, SortConfig } from '../search/search-filters/search-config.model';
...
import { FacetConfigResponse } from '../../../shared/samvad-ai/models/facet-config-response.model';

/**
 * Service that performs all actions that have to do with the current samvadAI configuration
 */
@Injectable()
export class SamvadAIConfigurationService implements OnDestroy {

  /**
   * Endpoint link path for retrieving samvad AI configurations
   */
  private configurationLinkPath = '/samvad-ai';
  ...
  /**
   * Emits the current samvadAI options
   */
  public samvadAIOptions: BehaviorSubject<SearchOptions>;

  /**
   * Emits the current samvadAI options including pagination and sort
   */
  public paginatedSamvadAIOptions: BehaviorSubject<PaginatedSearchOptions>;
  ...
  /**
   * Initialize the samvadAI options
  ...
  /**
   * Default values for the SamvadAI Options
   */
  ...
  /**
   * Creates an observable of SamvadAIConfig every time the configuration stream emits.
   * @param configuration The samvad-ai configuration
   * @param scope The samvad-ai scope if exists
   */
  getConfigurationSamvadAIConfig(configuration: string, scope?: string): Observable<SearchConfig> {
    return this.getSamvadAIConfigurationFor(scope, configuration).pipe(
      getAllSucceededRemoteDataPayload()
    );
  }

  /**
   * Return the SortOptions list available for the given SamvadAIConfig
   * @param samvadAIConfig The SamvadAIConfig object
   */
  getConfigurationSortOptions(samvadAIConfig: SearchConfig): SortOptions[] {
    return samvadAIConfig.sortOptions.map((entry: SortConfig) => ({
      field: entry.name,
  ...
  setPaginationId(paginationId): void {
    if (isNotEmpty(paginationId)) {
      const currentValue: PaginatedSearchOptions = this.paginatedSamvadAIOptions.getValue();
      ...
      this.unsubscribeFromSamvadAIOptions(this.paginationID);

      // change to the new pagination id
      this.paginationID = paginationId;
      this.paginatedSamvadAIOptions.next(updatedValue);
      this.setSamvadAISubscription(this.paginationID, this.paginatedSamvadAIOptions.value);
    }
  }
  ...
  /**
   * Initialize the samvad-ai options
   */
  protected initDefaults() {
    this.defaults
      ...
      .subscribe((defRD: RemoteData<PaginatedSearchOptions>) => {
        ...
        this.paginatedSamvadAIOptions = new BehaviorSubject<PaginatedSearchOptions>(defs);
        this.samvadAIOptions = new BehaviorSubject<SearchOptions>(defs);
        this.setSamvadAISubscription(this.paginationID, defs);
      });
  }

  private setSamvadAISubscription(paginationID: string, defaults: PaginatedSearchOptions) {
    this.unsubscribeFromSamvadAIOptions(paginationID);
    const subs = [
      this.subscribeToSamvadAIOptions(defaults),
      this.subscribeToPaginatedSamvadAIOptions(paginationID || defaults.pagination.id, defaults)
    ];
    this.subs.set(this.paginationID, subs);
  }

  /**
   * Sets up a subscription to all necessary parameters to make sure the samvadAIOptions emits a new value every time they update
   * @param {SearchOptions} defaults Default values for when no parameters are available
   * @returns {Subscription} The subscription to unsubscribe from
   */
  private subscribeToSamvadAIOptions(defaults: SearchOptions): Subscription {
    return observableMerge(
      ...
      const currentValue: SearchOptions = this.samvadAIOptions.getValue();
      const updatedValue: SearchOptions = Object.assign(new PaginatedSearchOptions({}), currentValue, update);
      this.samvadAIOptions.next(updatedValue);
    });
  }

  /**
   * Sets up a subscription to all necessary parameters to make sure the paginatedSamvadAIOptions emits a new value every time they update
   ...
   * @returns {Subscription} The subscription to unsubscribe from
   */
  private subscribeToPaginatedSamvadAIOptions(paginationId: string, defaults: PaginatedSearchOptions): Subscription {
    return observableMerge(
      this.getConfigurationPart(defaults.configuration),
   ...
   */
  private unsubscribeFromSamvadAIOptions(paginationId: string): void {
  ...
  /**
   * @returns {Observable<string>} Emits the current configuration settings as a partial SamvadAIOptions object
   */
  ...
  /**
   * @returns {Observable<string>} Emits the current query string as a partial SamvadAIOptions object
   */
  ...
  /**
   * @returns {Observable<string>} Emits the current pagination settings as a partial SamvadAIOptions object
   */
  ...
  /**
   * @returns {Observable<string>} Emits the current sorting settings as a partial SamvadAIOptions object
   */
  ...
  /**
   * @returns {Observable<Params>} Emits the current active filters as a partial SamvadAIOptions object
   */
  ...
  /**
   * @returns {Observable<string>} Emits the current fixed filter as a partial SamvadAIOptions object
   */
  ...
  /**
   * Request the samvad-ai configuration for a given scope or the whole repository
   ...
   */
  getSamvadAIConfigurationFor(scope?: string, configurationName?: string): Observable<RemoteData<SearchConfig>> {
  ...
  /**
   * @returns {Observable<Params>} Emits the current view mode as a partial SamvadAIOptions object
   */
